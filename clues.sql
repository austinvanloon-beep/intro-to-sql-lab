...
-- language classes in this country's officially recognized language. 
...
-- One is named the same as the country – that would be too obvious. We're following our gut 
-- on this one; find out what other city in that country she might be flying to.
...
WHERE id = (
  ...
  WHERE code = (
    SELECT countrycode
    FROM cities
    WHERE name ILIKE '%Serr%'
    AND countrycode <> (SELECT code FROM countries WHERE name = 'San Marino')
    LIMIT 1
  )
);
...
-- where she's heading and beat her to the landing dock. Lucky for us, she's getting cocky. 
-- She left us a note (below), and I'm sure she thinks she's very clever, but if we can crack it, 
